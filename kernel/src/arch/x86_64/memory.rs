...
fn init_kernel_kseg2_map(){
    ...
    debug!("Page table[510] before mapped: {}", unsafe{*(0xffff_ffff_ffff_fff0 as *const usize)});
    debug!("Page table[175] before mapped: {}", unsafe{*(0xffff_ffff_ffff_f578 as *const usize)});
    debug!("Page table[509] before mapped: {}", unsafe{*(0xffff_ffff_ffff_ffe8 as *const usize)});
    page_table.map(0xfffffe8000000000, 0x0).update();
    page_table.unmap(0xfffffe8000000000);
    debug!("Page table[509] after mapped: {}", unsafe{*(0xffff_ffff_ffff_ffe8 as *const usize)});
