...
use self::structs::*;
use crate::rcore_fs_sfs::structs::FileType::File;
...
impl vfs::INode for INodeImpl {
    ...
    fn metadata(&self) -> vfs::Result<vfs::Metadata> {
        ...
        Ok(vfs::Metadata {
            ...
            blocks: if disk_inode.type_==FileType::CharDevice {0} else {disk_inode.blocks} as usize,
            ...
            gid: 0,
            blk_size: BLKSIZE,
            rdev: if disk_inode.type_==FileType::CharDevice {((disk_inode.blocks as u64) << 32) | (disk_inode.size as u64)} else {0} as u64
    ...
    fn setrdev(&self, dev:u64)->vfs::Result<()>{
        ...
        disk_inode.blocks=(dev>>32) as u32;
        disk_inode.size=(dev&0xffffffff) as u32;
...
impl Drop for INodeImpl {
    ...
    fn drop(&mut self) {
        ...
        if self.disk_inode.read().nlinks <= 0 {
            // TODO: polyfill for the char device.
            if self.disk_inode.read().type_==FileType::CharDevice{
                //No need to resize, since always no block.
            }else{
